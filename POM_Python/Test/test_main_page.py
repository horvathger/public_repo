...
class TestMainPage:
    ...
    def test_login_locked_out_user(self):
        ...
        assert self.main_page.get_login_error_message().text == 'Epic sadface: Sorry, this user has been locked out.'
    ...
    def test_login_performance_glitch_user(self):
        self.main_page.do_login(PERFORMANCE_GLITCH_USER_LOGIN_DATA["username"], PERFORMANCE_GLITCH_USER_LOGIN_DATA["password"])
        assert self.logged_in_page.get_page_header().text == 'Swag Labs'
        assert self.logged_in_page.get_current_url() == self.logged_in_page.url
